...
\begin {equation}
  \begin {aligned}
    v_{t_{n+1}} &= v_{t_n} + \kappa (\theta - v_{t_n}) v_{t_n} \Delta t + \epsilon v_{t_n}^{3/2} \Delta W_n^v \&\quad + \frac {1}{2} \sigma_v (v_{t_n}) \sigma_v'(v_{t_n}) \left ( (\Delta W_n^v)^2 - \Delta t \right).
  \end {aligned}
\end {equation}
...
\begin {equation}
  v_{t_{n+1}} = v_{t_n} + \kappa (\theta - v_{t_n}) v_{t_n} \Delta t + \epsilon v_{t_n}^{3/2} \Delta W_n^v + \frac {3}{4} \epsilon^2 v_{t_n}^2 \left ( (\Delta W_n^v)^2 - \Delta t \right).
  \label {milstein_v}
\end {equation}
...
\end {equation}
将式 \eqref{ncx2_pg_mixture} 代入上式，可直接得到ytn+1​​的 Poisson-Gamma 混合表示：
\begin {equation}
...
\begin {equation}
  \Psi_n (x) =\begin {cases}x - a, & n = 0, \\frac {b-a}{n\pi} \sin\left ( \frac {n\pi (x-a)}{b-a} \right), & n \geq 1.\end {cases}
\end {equation}
\subsubsection{条件蒙特卡洛框架下的实现}
