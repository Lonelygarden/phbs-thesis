...
其中，$r$ 代表无风险利率，$W_t^1$ 和 $W_t^2$ 是两个相互独立的标准布朗运动，$\rho$ 是两个布朗运动之间的相关系数 。同时，方差过程 $V_t$ 的动态演进满足：
\begin{equation}
...
在3/2 模型的期权定价框架中，时间区间 $[0, T]$ 内的积分方差 (Integrated variance) $V_T$ 扮演着至关重要的角色 。其定义如下：
...
在风险中性测度 $\mathbb{Q}$ 下，3/2 模型的随机微分方程（SDE）由公式\eqref{eq:underlyingProcess}和公式\eqref{eq:varianceProcess}定义。

为了利用傅里叶变换方法进行期权定价，核心在于求解对数资产价格 $x_T = \ln S_T$ 的矩母函数（MGF）。

根据 Lewis (2000) 与 Carr 和 Sun (2007) 的推导，定义 $\phi(u, \tau) = \mathbb{E}[e^{u x_T} | \mathcal{F}_t]$ 为对数价格的矩母函数。在 3/2 模型框架下，由于其方差过程 $V_t$ 的倒数遵循 CIR 过程（平方根过程），该 MGF 并不具备仿射模型中常见的指数仿射形式，而是表现为包含库默尔合流超几何函数（Kummer's Confluent Hypergeometric Function, $_1F_1$）的解析形式。

具体而言，给定初始状态 $V_0 = \sigma_0^2$，定义辅助变量如下：参数变换项：$$\mu = \frac{1}{2} + \frac{\kappa - u \rho \epsilon}{\epsilon^2}, \quad \tilde{c} = \frac{u(1-u)}{\epsilon^2}$$

特征参数：$$\delta = \sqrt{\mu^2 + \tilde{c}}, \quad \alpha = -\mu + \delta, \quad \beta = 1 + 2\delta$$

时间相关项：定义 $$X = \frac{2\kappa \theta}{\epsilon^2 \sigma^2 (e^{\kappa \theta \tau} - 1)}$$

该项刻画了均值回归强度与期限 $\tau$ 对波动率结构的非线性影响。综上，3/2 模型下对数价格的矩母函数可表示为：
